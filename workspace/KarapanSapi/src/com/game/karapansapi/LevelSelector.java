...
public class LevelSelector extends Entity {
	...
	private final int TILE_DIMENSION = 100;
	private final int TILE_PADDING = 50;
	...
	public LevelSelector(final int pMaxLevel, final int pChapter,
		 ...
		 * center of the Scene
		 */
		final float halfLevelSelectorWidth = ((TILE_DIMENSION * COLUMNS) + TILE_PADDING * 2
				* (COLUMNS - 1)) * 0.5f;
		this.mInitialX = (this.mCameraWidth * 0.5f) - halfLevelSelectorWidth - 50 * 2;

		/* Same math as above applies to the Y coordinate */
		final float halfLevelSelectorHeight = ((TILE_DIMENSION * ROWS) + TILE_PADDING
				* (ROWS - 1)) * 0.5f;
		this.mInitialY = this.mCameraHeight - TILE_DIMENSION;
	}
	...
	public void createTiles(final ITextureRegion pTextureRegion, final Font pFont) {
		...
		float tempX = this.mInitialX + TILE_DIMENSION * 2 * 0.5f;
		...
		for (int i = 0; i < ROWS; i++) {
			...
			for (int o = 0; o < COLUMNS; o++) {
				...
				ITextureRegion xpTextureRegion;

				/* Determine whether the current tile is locked or not */
				if (currentTileLevel <= mMaxLevel) {
					locked = false;
					xpTextureRegion = ResourcesManager.getInstance().level_region;
				} else {
					locked = true;
					xpTextureRegion = ResourcesManager.getInstance().level_locked_region;
				}
				...
				LevelTile levelTile = new LevelTile(tempX, tempY, locked,
						currentTileLevel, xpTextureRegion, pFont);
				...
				tempX = tempX + TILE_DIMENSION * 2 + TILE_PADDING;
			...
			tempX = mInitialX + TILE_DIMENSION * 2 * 0.5f;
	...
	public class LevelTile extends Sprite {
		...
		public void attachText() {
			...
			final float textPositionX, textPositionY;
			...
			if (this.mTileText == null) {
				...
				if (this.mIsLocked) {
					...
					textPositionX = 0;
					textPositionY = LevelSelector.this.TILE_DIMENSION * 0.5f;
				} else {
					tileTextString = String.valueOf(this.mLevelNumber);
					/* Setup the text position to be placed in the center of the tile */
					textPositionX = LevelSelector.this.TILE_DIMENSION * 0.5f;
			...
			if (!LevelSelector.this.mHidden) {
				...
				if (pSceneTouchEvent.isActionDown()) {
					...
					if (this.mIsLocked) {
						...
						if(mLevelNumber == 1)
						{
							SceneManager.getInstance().loadGameScene(mEngine, 1);
							
						}
						if(mLevelNumber == 2)
						{
							SceneManager.getInstance().loadGameScene2(mEngine, 1);
							
						}
						if(mLevelNumber == 3)
						{
							SceneManager.getInstance().loadGameScene3(mEngine, 1);
						...
						if(mLevelNumber == 5)
						{
							SceneManager.getInstance().loadGameScene2(mEngine, 2);
							
						}
						if(mLevelNumber == 6)
						{
							SceneManager.getInstance().loadGameScene3(mEngine, 2);
							
						}
						if(mLevelNumber == 7)
						{
							SceneManager.getInstance().loadGameScene(mEngine, 3);
							
						}
						if(mLevelNumber == 8)
						{
							SceneManager.getInstance().loadGameScene2(mEngine, 3);
							
						}
						if(mLevelNumber == 9)
						{
							SceneManager.getInstance().loadGameScene3(mEngine, 3);
